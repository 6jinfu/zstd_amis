...
人才发展用于建立岗位和人才之间可复用的数据链：先明确组织结构与关键岗位，再建立可复用的人才标准，并通过动态应用范围覆盖岗位或人员条件；随后通过 360 评估和人才盘点形成能力与人才结果，最终在岗位地图和人才档案中使用这些结果。

当前版本聚焦管理端核心流程，不建设独立首页，不设置独立 AI 对话入口，也不在导航中展示延期功能。

### 1.2 当前需要解决的问题

1. 部门、岗位、人员、人才标准和人才结果分散，缺少统一关系。
...
- 标准作为独立业务记录配置适用对象；岗位关联标准，学习方案继续落在岗位。
...
- 人才标准、360 结果、盘点结果和人才详情使用一致的能力指标口径。
...
| 一级中心 | 顶部业务菜单 | 入口页面 |
| --- | --- | --- |
| 人才标准 | 组织结构 | `org-structure.html` |
| 人才标准 | 关键岗位分析 | `key-position.html` |
| 人才标准 | 人才标准 | `standards.html`、`standard-detail.html` |
| 人才标准 | 指标库 | `vocabulary.html` |
...
`index.html` 不承载首页内容，进入系统后直接跳转到人才标准。

### 3.2 延期或不在当前导航中的功能

| 功能 | 当前处理 |
| --- | --- |
| 模型库 / 岗位族独立模型 | 旧模型库延期；由人才标准承载标准与适用对象 |
...
- 左侧固定为一级中心：人才标准、评鉴中心、发展中心、人才中心、系统设置。
- 顶部显示当前一级中心下的业务菜单。
- 详情页、向导页和结果页继承所属中心的导航，不新增孤立入口。
- 桌面端左侧一级导航支持收起和展开：展开宽度为 `208px`，显示图标与名称；收起宽度为 `72px`，只显示品牌标识、一级中心图标和展开按钮。收起状态在同一浏览器中跨页面保留；所有图标入口必须保留可访问名称和悬停提示。移动端继续使用顶部横向一级导航，不显示侧栏收起按钮。
...
- 标准独立维护，通过指定岗位、人员条件或暂不关联定义适用范围；岗位可关联多套标准。学习方案继续落在岗位并明确采用一套标准。360 评估保持当前隐藏关联岗位等配置的范围，不新增页面。
...
- 能力指标可由企业创建、编辑、分类和分级，并可被人才标准、360 问卷和人才能力结果复用。
...
- 人才详情中的标准要求来自选定的匹配标准，实际能力来自最新有效结果；多套标准不自动合并，未选择标准时仅显示实际能力。
...
- 能力总分默认按百分制分档：`0%–60%` 为低、`60%–80%` 为中、`80%–100%` 为高；盘点规则仅支持设置能力轴区间数量，能力轴可选 `2–5` 档，绩效轴固定为上传数据中的低、中、高三档，不提供绩效分档数量修改。宫格总数上限仍为 `25` 格，当前配置实际最多为 `5×3=15` 格，首次默认 `3×3`。交换横纵轴时只交换两轴方向，不改变能力轴区间数量及绩效固定三档规则。
- 宫格展示文案可在盘点规则中逐格修改；区间数量变化时预览、文案编辑区和提交结果链接同步更新，项目创建后使用本次规则生成落位并锁定规格。
...
- 全站类别树面板（360 评估、人才测评、考试、人才盘点、关键岗位分析、指标库）统一规格：管理类别齿轮 32×32、类别标题 13px/600；齿轮图标一律用 360 评估同款完整 SVG（feather settings path + 新建旁列表设置按钮 SVG 15×15），不得使用残缺 path 或 ⚙ 字符。
- 全站类别树仅用于类别切换与筛选，类别名称旁不显示项目数、内容数或其他数字；数量统计只在列表、分页和类别管理抽屉等数据场景展示。
- 评鉴工具下的 360 评估、人才测评和考试类别树以人才盘点类别树为布局与样式基准：左栏宽 `208px`、内边距 `14px 12px`，标题区高 `40px`且不显示底部横线，类别项高 `40px`、内边距 `8px 10px`，工作区最小高度 `668px`；不得在评鉴工具页额外叠加专用横线、加高标题区或改变类别树位置。
- 全站横向二级 tab 统一规格（2026-08-26）：容器 min-height 46、tab 44、字号 13px、字重 500、间距 28px、容器左右 padding 4px、底部绘制区 1px、距内容 14px；容器底线为 1px `--border-subtle`，普通文字为 `--neutral-600`，选中文字为 `--primary-700`。选中横线统一使用贴合容器底线的 2px `--primary-600` 伪元素，切换时不改变字重，避免文字与横线跳动。可横向滚动的 tab 容器必须保留 1px 底部绘制区，禁止再次将 `bottom:-1px` 的高亮线裁成视觉 1px。适用页面级和内容级 `.tab-btns`、`.assessment-tool-tabs`、`.pm-tabs`、`.lm-tabs`；`card-tabs-solid` 由 card-head 承载通栏底线，但 tab 几何与选中横线保持一致。
- 抽屉（drawer）阴影只在 `.show` 打开态挂载；关闭态抽屉堆叠在视口右缘外，其 box-shadow 会渗入页面右缘形成灰蓝竖带（指标库页"透明遮罩"曾由此而来）。
- 静态资源引用带版本参数（`styles.css?v=` / `app.js?v=`），每次部署更新版本号，避免浏览器缓存旧版造成页面间"假性不一致"；HTML 保持不缓存，带版本参数的 CSS、JS 与图片字体资源允许长期缓存。

### BR-09 全局品牌色（2026-09-01）

- 全站品牌主色统一为 `#0152D9`，并使用同一蓝色色阶：浅底 `#F0F6FF`、浅底悬停 `#DCEAFF`、禁用 `#B8D5FF`、过渡色 `#85B7FF` / `#4D91FF`、悬停 `#1769F5`、默认 `#0152D9`、按下 `#0147BE`、深色 `#063B98` / `#07204E`。
- 主按钮必须按“默认 `#0152D9` → 悬停 `#1769F5` → 按下 `#0147BE`”呈现；禁用态使用 `#B8D5FF`，并配高对比深蓝文字。
- 导航选中态、页签、链接、焦点、图表蓝色与蓝色浅底均引用全局品牌色令牌，不再新增脱离色阶的蓝色值。
- 全站所有非数据、非状态的结构表面统一使用纯白：页面画布、顶部栏、一级导航、内容区、卡片、筛选区、类别树、表格表头和分页不再使用浅灰或浅蓝底色。区域层级通过低对比 hairline 分隔线、留白和必要阴影建立；品牌按钮、选中态、状态标签、图表及热力单元格继续保留对应业务颜色。
...
| ORG-02 | 矩阵横轴界面名称为“业务分工”（对应序列），纵轴界面名称为“业务分权”（对应层级），业务分权自下而上递增 | P0 |
| ORG-03 | 单元格展示岗位族名称、岗位数、人数和组编码 | P0 |
| ORG-04 | 支持综合视图、岗位评价法视图、结构分析法视图切换 | P1 |
| ORG-05 | 点击岗位族可查看组内岗位、AI 序列、AI 层级、岗位评价分和依据 | P0 |
| ORG-06 | 支持查看历史分析、查看方法说明和新建分析；操作按钮位于矩阵列表左上角（与全站列表主操作位置规范一致）。存在未完成任务时仍可新建，最多同时管理 3 个组织结构分析任务 | P0 |
| ORG-07 | 新建分析生成新版本草稿，不直接覆盖当前生效版本 | P0 |
| ORG-08 | 岗位族矩阵按覆盖人数从低到高划分 5 档热力颜色，固定使用 `#F0F6FF / #DCEAFF / #B8D5FF / #85B7FF / #4D91FF`；空宫格使用白色空态，不参与热力计算；矩阵外框和普通宫格分隔线使用主蓝 `#0152D9` 的 12% 透明度，已调整等业务状态边框可继续使用明确强调色 | P1 |
| ORG-09 | 组织结构页除岗位族热力单元格外统一使用白色背景；操作区、矩阵容器、行列标题、空宫格和统计区不得使用大面积浅灰或浅蓝底色。矩阵不显示外框和蓝色宫格线，以 4px 留白分隔单元；矩阵标题、统计区之间不叠加横线，统计指标不使用四边卡片框 | P1 |
| ORG-10 | 新建分析时允许用户配置组织结构矩阵规格：横轴业务分工（序列数）、纵轴业务分权（层级数）均为 1–5 的整数；所选数量是本次生成的硬性规格，宫格总数由系统自动计算，层级仍自下而上递增 | P0 |

#### 分析向导

> 向导保留**三步步骤条**（组织范围 → 说明材料 → 生成）。分析方法与维度配置仍由 Dify 使用内置参数；2026-09-14 起，“组织结构矩阵规格”在生成步开放用户配置，不增加新的向导步骤。

1. **组织范围**：在部门树中勾选范围，右侧同步展示岗位、类别和人数。
2. **说明材料**：上传岗位说明书、组织架构图等材料，展示解析状态，支持删除和重试。
3. **生成**：通过两组 1–5 分段选择器设置“横轴·业务分工”和“纵轴·业务分权”，底层分别对应序列数与层级数；右侧实时预览矩阵并显示“N 项业务分工 × M 级业务分权，共 N×M 宫格”；下方配置汇总同步显示本次规格，确认后开始分析。默认沿用当前生效版本规格，首次分析默认 3×3。

被移除的原步骤（Dify 内置默认口径）：

- 分析方法：默认岗位评价法 + 结构分析法（原可选结构设计法、岗位定义法、人效分析法）。
- 维度配置：默认按所选方法内置指标与权重。
- 归类方式：AI 在用户确定的矩阵规格内生成序列、层级名称并自动归类岗位，用户不在创建页手工编辑名称。

#### 结果预览与生效

- 点击“提交分析”后，创建接口须立即返回异步任务，不得等待 Dify 分析完成；当前页面弹出任务进度弹窗，分析在后台继续执行。用户可关闭弹窗并继续浏览其他页面，不强制停留在独立进度页。
- 分析过程显示排队、读取组织数据、按方法打分、推断序列/层级、归类岗位族、结果校验等业务阶段。
- 进度弹窗明确提供两种选择：留在弹窗持续等待，或点击“暂时关闭，返回分析页”继续处理其他页面；不得向用户暴露“异步保存”“草稿自动保存”等技术或冗余文案。关闭后不显示全局浮动提醒。组织结构主页始终保留当前生效矩阵或首次进入的空态、顶部“新建分析”和“分析记录”，不因后台任务运行而替换整页；运行中的任务以任务卡或分析记录展示，最多同时保留 3 个未终态任务。达到 3 个任务上限时，继续保留原任务，不自动取消，并提示用户先等待或处理现有任务。任务完成后再从任务记录进入结果预览。
- 用户新建分析时不复制其他运行任务的组织范围、说明材料或矩阵规格，始终从头重新选择；原有向导三步结构和当前生效版本的默认初始化规则保持不变。
- 仅当任务成功且结果校验通过后开放“完成后查看结果”；分析中不得提前进入结果预览。点击后先展示短暂的结果装载页，依次反馈读取结果、生成矩阵、准备校准，再进入结果预览。
- 预览页显示草稿矩阵、待人工归类岗位和多方法结论不一致项。
- 预览、当前结构、历史版本与关键岗位分析的引用版本均须按对应版本自身的序列数和层级数渲染；所有用户可见的矩阵轴标、规格预览、结果摘要和统计指标统一使用“业务分工 / 业务分权”，不得混用“序列数 / 层级数”；宽度不足时矩阵区域横向滚动，左侧业务分权标题在滚动中保持可见。
- 矩阵内没有岗位的单元格只显示“暂无岗位”，不得误标为“待人工归类”；未归类岗位继续放在矩阵外的独立待归类区。
- 用户可查看归类依据、选择最终归属或保留当前归类。
- 支持保存草稿和确认生效；确认生效后成为新的当前组织结构版本。

#### 验收要点

- 默认初始化没有分析结果时展示空状态和“开始分析”，不得预置样例矩阵；存在后台任务时空态与任务记录并存，不能阻塞新建分析。最多同时管理 3 个未终态任务，原任务不因新建任务自动取消。
- 调整后的岗位在矩阵中有明确标记，并可追溯调整依据。
- 未人工确认前，新结果不替换当前版本。
- 业务分工与业务分权分别选择 1 和 5 时，配置汇总、任务信息和结果矩阵均准确显示；任务提交后本次规格锁定，需要变更时必须新建分析。
...
- 列表页整体使用白色背景，不使用浅灰或浅蓝的大面积分区色；类别树、筛选区、表头与分页通过轻边线、留白和选中态主蓝区分层级。
- 支持按项目名、状态筛选。
- 列表字段：项目名、类别、引用组织结构、分析范围、所用视角、关键岗位数、状态、负责人、操作。
- 项目状态：草稿、排队中、分析中、校验中、已完成、分析失败、已取消；列表各状态均提供“编辑”入口，可编辑项目名、类别和分析目标。编辑只更新项目基本信息，不改变已提交的分析数据、引用组织结构、运行状态或分析结果；运行类状态仍提供查看进度和终止，只有已完成状态提供查看结果。
- 编辑流程：列表操作栏点击“编辑”打开弹窗，修改项目名、类别和分析目标后保存返回列表；不进入新建页，也不重新触发分析。
...
- 底部固定操作区：取消、提交分析。
- 点击提交分析后立即创建后台任务并在当前页面打开进度弹窗，不得在创建请求中同步等待 Dify 返回，也不得强制跳转或停留在独立进度页。弹窗元素必须与表单和引用版本数据一致：项目名、引用组织结构版本摘要、双视角口径，进度阶段为排队 → 读取组织版本数据 → 经济视角打分 E → 能力视角打分 C → 合成关键系数与理由 → 结果校验。
- 进度弹窗允许用户留在页面持续等待，也提供“暂时关闭，返回分析页”；关闭后返回项目列表且任务继续运行，不显示全局浮动提醒。不得向用户展示“异步保存”等技术文案。列表中的运行任务提供“查看进度”并在当前页重开弹窗，不得显示可用的“查看结果”。任务成功且结果校验通过后才开放“完成后查看结果”，点击后先展示短暂装载状态，再进入结果页。
...
- 对建议关键岗位确认后，更新岗位的关键标记；用户可进入岗位详情查看适用标准或学习方案，标准内容统一回到人才标准维护。
...
岗位数据页与岗位详情继续保留，供组织分析结果、人才标准应用范围和学习方案使用；它不再作为人才标准中心中的并列主入口，标准内容和应用范围统一在 6.5 人才标准中维护。
...
- 行操作：编辑、适用标准、学习方案；适用标准仅进入岗位上下文查看和关联已有标准。
- 岗位列表提供“销售经理（SA-201）”销售岗位演示案例；从该行进入岗位详情后，岗位信息、适用标准、能力选项、学习方案和 AI 配课结果均使用销售业务口径，不得回落为算法工程师数据。
...
2. **关联标准**：查看、新建、关联及解除独立标准；另外展示当前可用在岗人员匹配的条件标准。
3. **学习方案**：关联课程、项目、前测和后测。

标准内容沿用任职、资格编辑，统一在标准详情维护：
...
- 支持移除已关联内容。
- “销售经理”案例的适用标准覆盖客户洞察、顾问式销售、商务谈判、商机管理和协同推进等能力，学习方案推荐销售课程、销售训练项目及销售能力前后测，形成可完整点击演示的关联与配课链路。
...
| 类型 | 企业权限 | 是否分级 | 主要用途 |
| --- | --- | --- | --- |
| 能力 | 可新增、编辑、删除、分类 | 可选行为等级 | 人才标准、360 评估、人才能力 |
| 动力 | 只读固定库 | 不分级 | 专家人才测评 |
| 潜力 | 只读固定库 | 不分级 | 专家人才测评 |

能力列表支持类别树、名称查询、类别管理、新建能力和分页。列表字段仅为名称、类别、描述和操作，不展示“分级”“等级数”等指标库字段；能力要求使用的等级仍在人才标准内容中按要求配置，不把等级维护状态重复放回指标列表。

能力指标选择使用全屏三栏穿梭框：左侧为能力指标类别树，中间为待选指标与名称查询，右侧为已选指标，底部固定取消 / 确认。类别搜索提示直接放在搜索框内（如“搜索能力指标类别”），不在输入框外重复显示。能力类别树演示数据按“技能、知识、素养、新品导入与开发”分组，并支持技能下的管理能力、通用能力、销售能力建设、专业能力（技术簇、生产簇、质量簇、财务簇、市场簇、客服簇、行政簇、采购簇、人力资源簇、产品簇、销售簇、测试）和云端技能等层级展开。类别节点不显示文件夹或文件图标，只有可展开节点显示必要的展开箭头，叶子节点预留同宽对齐位。动力、潜力的标准内容选择同样使用该穿梭框结构，并使用各自固定指标类别。

能力、动力、潜力三个页签的列表样式直接复用岗位管理列表规范：类别栏固定为 208px，工作台撑到视口底部，表格区自适应占满剩余高度并在内部滚动，分页固定在工作台底部；表格不增加页面专用外框、固定高度、固定列宽或固定操作列宽，横向溢出必须由表格容器承担滚动。“新建能力”沿用岗位管理的主操作区，位于筛选区下方。三个页签的类别项统一行高、内边距、选中态、悬停态、计数位置和点击筛选交互，不显示文件夹、文件等类别图标；能力类别因存在层级，额外提供同样风格的展开/收起和缩进，动力、潜力仍使用扁平类别。三个页签仅保留业务权限、主操作和字段上的必要差异。

动力、潜力的词条查看抽屉仅展示名称、类别和描述，不再展示“稳定特质、不分级、高低由评鉴中心体现”等额外提示语。

---

### 6.5 人才标准（2026-09-11）

人才标准是标准内容与动态应用范围的唯一维护入口，不再将岗位管理与标准管理作为两个并列入口。新增 `standards.html` 和 `standard-detail.html`，不引入角色库或对象中心；多等级、复杂认证规则与全面内容结构改造不在本轮实现范围。

#### 标准列表

- 入口位于人才标准中心，组织结构和关键岗位分析作为前置对象分析入口保留。
- 沿用岗位列表工作台：208px 类别栏、筛选区下方新建按钮、表格内部滚动、固定底部分页。动态列表使用原生 sticky 操作列，避免旧冻结列复制脚本使操作失效。
- 支持标准名称查询、类别筛选、真实条数与每页 10 条分页。
- 列表字段仅为：标准名称、适用对象、操作。适用对象列展示实际岗位名称或条件值，不显示纯数字数量；暂不关联时显示“—”。具体配置仍在详情中查看和维护。
- 类别继续通过左侧类别树管理，不在表格中重复展示类别列；演示类别为通用标准、岗位序列标准（产品序列、研发序列、销售序列、专业序列）、职级标准、专项标准；岗位序列标准支持展开/收起，子类别相对父类别明显缩进。类别节点不显示文件夹、文件或其他类别图标；不可展开节点预留与展开控件同宽的对齐位，只有父节点显示必要的展开箭头。点击任一类别始终执行筛选，点击展开箭头只执行展开/收起；支持编辑、复制、删除。
- 类别可新增、改名、删除；非空类别不得删除，类别只用于归档而不参与人员匹配。
- 复制只带入标准内容和标准说明，清空应用范围；被岗位或下游学习方案采用的记录不得直接删除。

#### 标准详情与对象配置

- 沿用左侧页内导航，直接平铺基本信息、应用范围、职责任务、经历经验、能力要求、动力要求、潜力要求七个入口；详情主体撑满可用高度，底部固定取消/保存操作区，长内容滚动时不得遮挡最后一段内容。
- 基本信息：标准名称（必填）、类别（必填）、标准说明。标准名称只用于识别和维护标准，不再重复维护“对象名称”。
- 新标准可以先保存内容，再在应用范围中选择岗位、组合条件或暂不关联；不要求先建岗位才能保存标准。
- 三种方式互斥：指定岗位（多选）、按条件匹配、暂不关联。切换方式后保存时清理失效的原方式条件。
- 条件组内 AND、组间 OR、同字段多值 OR。支持增加/删除组、增加/删除条件、选择字段及多选值。
- 本轮字段为部门、现岗位、岗位类别、关键岗位，来源为现有岗位列表和人才列表及其部门归属。部门为明确枚举，不暗含下级组织；暂不开放职种、职系、专业方向及 AI 分析序列/层级自动匹配。
- 条件为空不可保存，允许零匹配保存。缺失属性采用三态匹配：匹配、不匹配、待确认；未知属性不按否或空值强行判为不匹配。
- 应用范围编辑只维护岗位选择或人员条件组合，不在编辑页额外展示可用人数、匹配人数或“查看匹配人员”摘要；匹配结果由岗位详情、人才档案等使用场景按需呈现。
- 标准内容分为职责任务、经历经验、能力要求、动力要求、潜力要求五个分项，支持编辑、增删、指标选择；不设置“引用已有标准”入口，避免把旧岗位标准复制模型带入独立标准维护。原算法和销售示例由共享数据承接。当前编辑器维护一个最多 20 项能力的分组；多分组上限仍是后续内容编辑扩展目标，不伪装成已实现。
- 芯片、广告是来自参考资料的标准示例，内容待维护，不宣称已完成客户标准导入。
- 原型通过 localStorage 保存标准、类别、显式关联、采用标准选择与学习方案资源；不修改现有原始指标定义。

#### 岗位与下游引用

- 岗位详情保留适用标准的查看与跳转上下文，不新增第二套标准内容维护入口；标准名称、应用范围和标准内容统一回到人才标准列表与详情维护。
- 应用范围支持指定一个或多个岗位、按人员条件组合匹配或暂不关联。指定岗位和条件匹配均由同一人才标准记录承载。
- 条件匹配标准按已知在岗人员展示，不能仅凭某一员工匹配就推广到整个岗位；查看匹配人员可核查依据。
- 人才档案张明案例从岗位关联与条件匹配生成候选。多候选要求明确选择，不自动合并；要求标记按所选标准重绘，指标缺失不画要求线，实际能力值保持原来源。
- 学习方案采用岗位适用标准。原有课程、项目和前后测选择保留，推荐按所选能力匹配已有资源；无能力要求时不开放推荐，未匹配资源可手动添加。
- 本轮不新增职级体系、不恢复旧模型库、不扩展 360 配置或认证执行。

#### 验收

1. 无岗位创建标准，刷新后内容保留。
2. 多岗位关联同一标准，编辑内容后各入口引用一致。
3. 部门=研发中心且关键岗位=是匹配张明、李华；加入岗位类别条件后缺少该属性者进入待确认。
4. 多组 OR 条件可覆盖不同群体；不完整规则显示校验反馈。
5. 人才档案切换标准时要求线变化，实际结果不变；学习方案保存后刷新保留采用标准与资源。
6. 列表、详情、选择抽屉沿用品牌色、白底、轻边框、键盘可用和移动端可访问规范。
...
3. **盘点活动**：选择已结束的 360 评估或考试项目，并上传与盘点人员匹配的低、中、高绩效数据；绩效等级固定为低、中、高，不在上传环节扩展分档。
4. **盘点规则设置**：选择能力与绩效的横纵轴方向，设置能力轴区间数量（`2–5` 档；绩效轴固定为低、中、高三档，宫格总数上限仍为 `25`，当前实际最多 `5×3=15` 格，默认 `3×3`），调整能力各档的人员占比，并逐格编辑宫格文案。能力按得分从高到低排序，默认高档选取前 `20%`、中档选取接下来 `20%`、低档为其余 `60%`；每档最低 `5%`，调整任意一档时另外各档自动平衡，合计始终为 `100%`。页面在分档条和各档控件中展示排名区间与按当前盘点人数换算的预估人数，不再额外展示重复的 TOP 高档摘要或规则解释文案。绩效沿用上传数据中的低、中、高有序等级，不提供绩效分档数量修改。

结果页沿用组织结构矩阵的查分方式：矩阵单元格展示落位名称和人数，点击单元格打开右侧人员明细抽屉，展示该格的横纵轴区间、匹配人数、姓名、部门、岗位、能力得分、绩效和落位；下方人员清单同步按单元格筛选，支持“查看全部”恢复。

保存草稿时不锁定数据；确认创建后生成初始落位并进入进行中状态。盘点人员、项目数据、绩效数据和盘点规则锁定。

- 列表中的“销售铁军盘点”是可点击的已结束宫格盘点案例，人员范围为销售中心；案例中的项目数据、绩效数据、人员清单、校准任务和人才池均使用销售业务口径，岗位只包含大区销售总监、战略客户总监、区域销售经理、大客户销售经理、渠道经理、销售顾问等销售相关岗位。
...
- 方案落在具体岗位，不落在岗位族；采用标准从岗位显式关联中选择，选择与方案资源持久化。推荐资源依据所选标准的能力要求，不使用固定岗位差距替代。
- 支持关联课程、培训项目、前测和后测。
- 课程和项目复用培训平台数据。
- 前测、后测复用已有考试逻辑。
...
- 标准候选来自人员条件匹配及当前岗位的显式关联；用户选择一套作为要求依据。
- 实际能力按最新有效考试或评估结果汇总，同一能力取最高等级。
- 未配置标准要求的能力不展示要求线；未选择标准时不显示要求线；无有效结果的能力不伪造实际值。
...
```text
配置草稿 → 排队中 → 分析中 → 校验中 → 结果预览 → 已生效
                     └────────→ 分析失败 / 已取消
```

- 排队中、分析中和校验中均为异步后台状态，用户离开页面不影响任务继续执行。
- 组织结构分析最多同时管理 3 个未终态任务；新建第 2、3 个任务时原任务继续运行，不自动取消。达到上限后不得创建第 4 个任务。
- 新建分析不复制其他运行任务的配置，用户需从头重新选择组织范围、说明材料和矩阵规格。
- 预览阶段允许校准和保存草稿。
- 只有确认生效后才更新当前组织结构。
...
- 运行中的项目从列表进入进度页；只有已完成项目可进入结果页。
- 任一状态的项目都可从列表编辑项目名、类别和分析目标；编辑不改变任务状态、已提交分析数据、引用组织结构或分析结果。
...
| Position | 编号、名称、类别、岗位族、关键标记 | 关联独立标准、学习方案、地图节点 |
...
| 人才标准 | 独立维护标准内容，并配置动态应用范围 | 用户编辑、选择并保存 |
| 学习方案 | 根据岗位适用的人才标准推荐课程和项目 | 用户添加、移除并保存 |
...
- 运行失败时，页面仅展示业务化错误和新建分析入口，不展示内部堆栈、密钥或节点实现。
...
- 左树只承载范围选择，不重复放置筛选表单。
- 列表工作区高度和分页位置保持一致；数据不足时由列表区域留白，分页器固定在工作区底部。
- 表格内容默认单行；列多或文字长时允许横向滚动。
- 桌面端操作列固定在最右侧，横向滚动时保持可见；窄屏不复制冻结列，随完整表格横向滚动，避免操作列挤占主要数据空间。
...
- 2026-09-10：一级业务中心统一命名为“人才标准”；原岗位管理与标准管理不再作为并列入口，人才标准列表成为标准唯一维护入口。列表只展示标准名称、适用对象数量和操作，适用方式与具体岗位 / 条件仅在详情的“应用范围”中查看和维护。
- 2026-09-10：标准详情左侧导航将基本信息、应用范围及职责任务、经历经验、能力要求、动力要求、潜力要求七个入口统一平铺，不再使用“标准内容”父级层级；右侧按分项维护，统一业务文案并减少长页面堆叠。
- 2026-09-04：产品品牌入口恢复正式图形标识，不再以“AI”文字充当 Logo；生成类主操作使用“生成岗位标准”“推荐学习资源”等任务结果命名，AI 仅作为过程能力说明。
- 2026-09-04：页面视觉以一个主工作表面为核心，内部优先使用留白和细分隔线，减少同层级白卡、重复圆角和阴影；常规交互目标至少提供 44px 可点击范围。
- 2026-09-04：岗位管理沿用顶部岗位中心与岗位管理导航表达页面身份，不额外增加重复页标题；岗位名称可进入详情，同时保留明确的“编辑、岗位标准、学习方案”行操作；关键岗位视图不重复展示“关键岗位”状态列；列表工作台保持统一高度，分页器固定在底部。
- 2026-09-04：系统设置由顶部业务导航表达“职级管理／功能设置”页面身份，内容区不重复展示“职级管理”或“学员端设置”标题，只保留必要说明、操作和配置项。
- 2026-09-04：组织结构与关键岗位的分析进度弹窗、结果装载过渡页顶部不展示 `Background task`、`Result ready` 等英文标题，直接使用中文业务标题和状态。
- 2026-09-04：组织结构当前结果页点击岗位族宫格后，在岗位列表操作栏保留可用的“查看依据”；点击后直接展示该岗位的岗位评价法、结构分析法、建议归属和归类说明，不要求跳转到结果预览页。
...
- 同源页面功能切换不得先显示旧版导航骨架或整页白闪；共享 Shell 和列表骨架应在首次绘制前完成初始化。支持跨页面过渡的 Chrome 使用 140ms 纯透明度淡入，不使用位移，并在用户启用“减少动态效果”时关闭动画。
...
2. 点击新建分析，完成三步配置（组织范围、说明材料、生成）并提交后台分析任务，创建请求立即返回。
3. 提交后可关闭进度弹窗并回到组织结构主页；任务继续执行，主页保留当前矩阵或空态、“新建分析”和“分析记录”。最多同时管理 3 个未终态任务，完成前不能进入结果预览。
4. 查看归类依据，校准存在分歧的岗位。
5. 确认生效后返回组织结构并看到新版本。

### AC-02 关键岗位与人才标准

1. 用户创建关键岗位分析，选择组织版本并提交后台分析任务，创建请求立即返回。
2. 用户可从列表任一状态的操作栏进入编辑，修改项目名、类别和分析目标并保存；列表内容和类别筛选同步更新，任务状态、引用组织结构和分析结果保持不变。
3. 用户可返回列表继续其他操作，并通过“查看进度”重新进入；任务完成前不能查看结果。
4. 对建议关键岗位人工确认，对分歧岗位校准。
5. 进入岗位管理，在关键岗位视图看到已标记岗位。
6. 进入岗位详情，查看适用标准并完成学习方案配置；标准内容回到人才标准入口维护。
...
2. 创建宫格盘点时，用户填写基本信息、选择盘点人员、选择已结束的测评项目、上传绩效数据，并设置能力与绩效轴向、能力轴区间数量及宫格文案；能力轴区间数为 `2–5`，绩效轴固定为低、中、高三档，宫格总数上限为 `25`（当前实际最多 `5×3=15` 格），默认 `3×3`。
3. 宫格盘点创建后按锁定的规格生成初始落位；结果页按同一规格展示矩阵，点击单元格可打开人员明细抽屉并同步筛选人员清单。
...
| 模块 | 页面 |
| --- | --- |
| 人才标准 | `org-structure.html`、`org-analyze.html`、`org-analyzing.html`、`org-preview.html` |
| 人才标准 | `key-position.html`、`key-position-new.html`、`key-position-analyzing.html`、`key-position-result.html` |
| 人才标准 | `positions.html`、`position-detail.html`、`vocabulary.html` |
...
这些页面不得出现在当前导航中；文件仅保留轻量兼容跳转，不保留延期功能的 UI 实现，直接访问时按本 PRD 第 3.2 节跳转。


### 原型前端公共交互补充（2026-09-08）

- 搜索多选共用选择器：搜索不清空勾选，显示已选数量，超限时保留选择并在抽屉内提示；取消不提交。
- 标准、评鉴项目、指标库、关键岗位与盘点列表的类别抽屉统一采用保存后生效，取消放弃；非空类别不可删除。
- 岗位与独立学习方案共用资源编辑：重复资源不重复添加，可移除全部并显示空态；选择标准实时预览，底部保存后生效，取消不提交本次方案编辑。
- 资源动作统一为添加课程、添加项目、添加前测、添加后测、推荐学习资源。
- 列表操作列统一采用原生 sticky。2026-09-08 最新用户要求仅关注桌面端，移动端不作为本轮优化或验收范围。
- 本轮桌面重点优化及关键回归已完成，详细实测范围见 `deliverables/frontend-review/2026-09-08-review.md`。为节省额度采用代表页面视觉抽验，不将未逐项验证的交互态标为通过。
