...
- 进度弹窗明确提供两种选择：留在弹窗持续等待，或点击“暂时关闭，返回分析页”继续处理其他页面；不得向用户暴露“异步保存”“草稿自动保存”等技术或冗余文案。关闭后不显示全局浮动提醒。首次进入组织结构页默认展示无数据缺省状态，仅保留“开始分析”，不提供样例数据捷径；开始分析并返回组织结构页后展示后台 Loading 状态，等待区同时呈现岗位族矩阵构建动画、当前处理阶段和整体进度；任务完成后用户刷新页面呈现岗位族矩阵。组织结构页通过“分析记录”展示运行中及历史任务，“查看进度”在当前页面重新打开弹窗。
...
- 默认初始化没有分析结果时展示空状态和“开始分析”，不得预置样例矩阵；异步保存后展示后台 Loading，任务完成并刷新后切换为结果矩阵，并持久化“已有组织分析结果”状态。
...
- 项目状态：草稿、排队中、分析中、校验中、已完成、分析失败、已取消；运行类状态提供查看进度和终止，只有已完成状态提供查看结果。当前不设置关键岗位分析编辑页，列表不提供“编辑”操作。
...
4. **盘点规则设置**：选择能力与绩效的横纵轴方向，调整能力低／中／高三档的人员占比，并逐格编辑宫格文案。能力按得分从高到低排序，默认高档选取前 `20%`、中档选取接下来 `20%`、低档为其余 `60%`；每档最低 `5%`，调整任意一档时另外两档自动平衡，合计始终为 `100%`。页面必须同时展示排名区间、“前百分之几”的人员选取口径，以及按当前盘点人数换算的预估人数。绩效沿用导入数据中的低／中／高等级，不另设占比。
...
3. 在进度页离开后任务继续执行；从“分析记录”重新进入可看到最新进度，完成前不能进入结果预览。
4. 查看归类依据，校准存在分歧的岗位。
5. 确认生效后返回组织结构并看到新版本。
...
2. 用户可返回列表继续其他操作，并通过“查看进度”重新进入；任务完成前不能查看结果。
3. 对建议关键岗位人工确认，对分歧岗位校准。
4. 进入岗位管理，在关键岗位视图看到已标记岗位。
5. 进入岗位详情，查看适用标准并完成学习方案配置；标准内容回到人才标准入口维护。
