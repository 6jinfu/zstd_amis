...
- 行操作：编辑、岗位标准、学习方案。
- 岗位列表提供“销售经理（SA-201）”销售岗位演示案例；从该行进入岗位详情后，岗位信息、AI 创建岗位标准、能力选项、学习方案和 AI 配课结果均使用销售业务口径，不得回落为算法工程师数据。
...
- 支持移除已关联内容。
- “销售经理”案例的岗位标准覆盖客户洞察、顾问式销售、商务谈判、商机管理和协同推进等能力，学习方案推荐销售课程、销售训练项目及销售能力前后测，形成可完整点击演示的建模与配课链路。
...
能力、动力、潜力三个页签的列表样式直接复用岗位管理列表规范：类别栏固定为 208px，工作台撑到视口底部，表格区自适应占满剩余高度并在内部滚动，分页固定在工作台底部；表格不增加页面专用外框、固定高度、固定列宽或固定操作列宽。“新建能力”沿用岗位管理的主操作区，位于筛选区下方。三个页签仅保留业务权限、主操作和字段上的必要差异。
