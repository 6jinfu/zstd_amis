...
- 每次新建分析创建新快照、新 `run_id` 和新版本，不覆盖旧结果。
...
- 组织结构分析最多同时存在 3 个未终态运行任务（`queued`、`snapshotting`、`running`、`validating`）；新任务不得自动取消原任务。达到上限时创建接口返回业务错误，前端保留原任务并提示用户。
- 用户侧“新建分析”始终重新提交组织范围、材料和矩阵规格，不复制其他运行任务的配置；服务端内部因网络失败进行的重试不改变这一交互约束。
...
```json
{
  ...
  "grid_config": { "sequence_count": 3, "level_count": 4 }
}
```

`grid_config.sequence_count` 与 `grid_config.level_count` 均为必填整数，取值范围 `1–5`，分别表示横轴序列数（界面文案“业务分工”）与纵轴层级数（界面文案“业务分权”）。该值是本次运行的硬性结果规格，创建并入队后不得修改；超出范围或非整数时返回 `400 Bad Request`。
...
当已有 3 个组织结构分析任务处于未终态时，创建接口返回 `409 Conflict`，错误码建议为 `ORG_ANALYSIS_CONCURRENCY_LIMIT`；该错误不取消或改变已有任务。
...
`PATCH /api/talent/v1/key-position-analysis/runs/{run_id}`

```json
{
  "name": "2026 年度关键岗位分析",
  "category_id": "annual",
  "objective": "识别需要优先培养的关键岗位"
}
```

仅更新项目名、类别和分析目标，不修改已提交的快照、引用组织结构、分析配置、运行状态或分析结果；成功返回 `200 OK`。该编辑能力不复制或重新提交原任务，若需再次分析，用户应从头新建任务。
...
{
  "schema_version": "1.1",
  "sequences": [{ "code": "RD", "name": "研发", "order": 1, "reason": "..." }],
  "levels": [{ "code": "L1", "name": "初级", "order": 1, "reason": "..." }],
  "position_groups": [{
    "group_code": "G-RD-3",
    "sequence_code": "RD",
    "level_code": "L3",
...
`sequences` 和 `levels` 的元素数量必须分别等于请求中的 `sequence_count` 和 `level_count`。`order` 从 `1` 开始且连续不重复；序列按从左到右排列，层级 `order=1` 为最低层级、界面上自下而上递增。空宫格允许不出现对应 `position_groups` 记录，但所有已归类组的 `sequence_code` 与 `level_code` 必须引用本次输出中存在的轴值。
...
输出中的岗位只能引用当前 `snapshot_id` 内的 `source_id`。出现未知岗位、重复岗位、分数越界、序列/层级数量与请求规格不一致、`order` 非法、缺少必填字段或 Schema 版本不兼容时，本次运行进入 `failed`，不得产生业务版本。

## 7. 重试、审计与人工校准

- 网络失败或 Dify 5xx：服务端指数退避重试 3 次；仍失败则标记失败。前端不提供复制原任务配置的“重新分析”，用户如需再次发起，应通过“新建分析”从头选择信息和数据。
- 业务输出校验失败：不自动重试，记录原始输出与校验错误，便于修复工作流。
- 每次人工校准记录调整前值、调整后值、操作人、时间和原因。
- 生效版本不可直接覆盖；再次新建分析或重新校准产生新修订记录。
